class { 'ffnord::params':
  router_id => "10.115.6.1",	  # The id of this router, probably the ipv4 address
...
ffnord::mesh { 'mesh_ffcux':
  ...
  , fastd_secret => "/root/gw04-fastd-secret.key"	
  , fastd_port   => 10050
  ...
  , dhcp_ranges => ['10.115.6.2 10.115.7.254'] 	# the whole net is 10.71.0.0 - 10.71.63.255 
						# so take one 32rd of this range but don't give out the ip of the gw itself
  , dns_servers => ['10.115.6.1']   		# should be the same as $router_id
}
